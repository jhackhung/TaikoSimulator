...
create_button PROC
    ...
    ; ?µÂª∫‰ΩçÁΩÆ?ëÈ? !Á®ãÂ?‰ΩçÁΩÆÂ≠òÂú®ebp+4
    push dword ptr [ebp+12] ; y Â∫ßÊ?
    push dword ptr [ebp+8]  ; x Â∫ßÊ?
    push esi
    call sfRectangleShape_setPosition
    add esp, 12

    ; Ë®≠Â?Â§ßÂ?
    push dword ptr [ebp+20] ; È´òÂ∫¶
    push dword ptr [ebp+16] ; ÂØ¨Â∫¶
    push esi
...
init_buttons PROC
    ...
    ; ‰øÆÊîπÂ∫ïÈÉ®?∑ÊñπÂΩ¢È??≤Â??äÊ?
    push beige_color  ; ÈµùÈ???
    push dword ptr [instruction_shape]
    call sfRectangleShape_setFillColor
    add esp, 8

    push black_color  ; ÈªëËâ≤?äÊ?
    push dword ptr [instruction_shape]
    call sfRectangleShape_setOutlineColor
    add esp, 8
...
@key_1_pressed:
    mov dword ptr [KeyA_state], 1 ; Ë®≠Â??Ä?ãÂ∑≤?â‰? 
    call play_music1
...
@key_2_pressed:
    mov dword ptr [KeyS_state], 1 ; Ë®≠Â??Ä?ãÂ∑≤?â‰?
    call play_music2
...
@key_3_pressed:
    mov dword ptr [KeyD_state], 1 ; Ë®≠Â??Ä?ãÂ∑≤?â‰?
    call play_music3
